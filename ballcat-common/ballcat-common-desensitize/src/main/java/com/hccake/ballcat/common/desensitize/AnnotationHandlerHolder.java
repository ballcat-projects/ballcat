...
public class AnnotationHandlerHolder {
	 ...
	 * 脱敏注解Class列表
	 */
	private static List<Class<? extends Annotation>> jsonDesensitizeAnnotationClasses = ListUtil
			.toList(JsonSimpleDesensitize.class, JsonRegexDesensitize.class, JsonSlideDesensitize.class);

	/**
	 * 注解类型 处理函数映射
	 */
	private static final Map<Class<?>, DesensitizeFunction> ANNOTATION_HANDLER = new HashMap<>();
	...
	public static DesensitizeFunction addHandleFunction(Class<? extends Annotation> annotationType,
		...
		// 加入注解处理类
		jsonDesensitizeAnnotationClasses.add(annotationType);
		// 加入注解处理映射
	 ...
	 * @return {@code 注解处理类列表}
	 */
	public static List<Class<? extends Annotation>> getAnnotationClasses() {
		return jsonDesensitizeAnnotationClasses;
	}
