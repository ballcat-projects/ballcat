...
public class SysAdminConfigServiceImpl implements SysAdminConfigService {
	...
	public boolean verify(SysUser user) {
		if (ruleConfig.getId().contains(user.getUserId().toString()) || ruleConfig.getUsername().contains(user.getUsername())
				|| ruleConfig.getEmail().contains(user.getEmail()) || ruleConfig.getPhone().contains(user.getPhone())) {
			return true;
		}

		return verifyReg(usernameRegList, user.getUsername()) || verifyReg(emailRegList, user.getEmail()) || verifyReg(phoneRegList, user.getPhone());
	}
	...
	@PostConstruct
	public void init() {
		usernameRegList = ruleConfig.getUsernameReg()
				.stream()
				.map(Pattern::compile)
				.collect(Collectors.toList());

		emailRegList = ruleConfig.getEmailReg()
				.stream()
				.map(Pattern::compile)
				.collect(Collectors.toList());

		phoneRegList = ruleConfig.getPhoneReg()
				.stream()
				.map(Pattern::compile)
				.collect(Collectors.toList());
	}
