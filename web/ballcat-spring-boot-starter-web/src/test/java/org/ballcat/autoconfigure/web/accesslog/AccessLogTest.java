...
class AccessLogTest {
	...
	@BeforeEach
	public void setup() {
		mockMvc = MockMvcBuilders.webAppContextSetup(context).addFilters(accessLogFilter).build();
	}

	@Test
	void testDefault() throws Exception {
		TestAccessLogFilter testAccessLogFilter = (TestAccessLogFilter) accessLogFilter;
		testAccessLogFilter.clearHttpInfo();

		// ’/test‘ 走默认选项。只记录 queryString, 不记录请求体和响应体
		HttpInfo httpInfo = testAccessLogFilter.getHttpInfo();
		assertNull(httpInfo);
		mockMvc.perform(MockMvcRequestBuilders.get("/test?a=1"))
	...
	void testIgnored() throws Exception {
		...
		HttpInfo httpInfo = testAccessLogFilter.getHttpInfo();
		assertNull(httpInfo);
		mockMvc.perform(MockMvcRequestBuilders.get("/test/ignored?a=2"))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.content().string("Hello, World!"));
	...
	void testAnnotation() throws Exception {
		...
		String prefix = "haha";
		mockMvc.perform(MockMvcRequestBuilders.post("/test/annotation?a=3").content(prefix))
			.andExpect(MockMvcResultMatchers.status().isOk())
	...
	void testRecordAll() throws Exception {
		...
		String prefix = "haha";
		mockMvc.perform(MockMvcRequestBuilders.post("/test/record-all?a=4").content(prefix))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.content().string(prefix + " Hello, World!"));
		httpInfo = testAccessLogFilter.getHttpInfo();
		assertNotNull(httpInfo);
