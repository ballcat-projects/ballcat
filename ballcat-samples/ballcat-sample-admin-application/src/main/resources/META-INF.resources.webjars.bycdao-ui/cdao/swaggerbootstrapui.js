...
(function ($) {
    ...
    * 获取全局缓存auth信息
    ...
    SwaggerBootstrapUi.prototype.createApiInfoInstance=function(path,mtype,apiInfo){
        ...
        // host如果不是协议开始，则直接默认使用当前host, 用相对路径
        host= host.startWith("http")? host: '';
        // 添加basePath
        var basePath=that.currentInstance.basePath;
        // 如果非空,非根目录
        var basePathFlag= basePath!=""&&basePath!="/";
        // 相对路径
        var relativePath = basePathFlag? basePath + path : path;
        //that.log("新的url:"+newurl)
        relativePath= relativePath.replace("//","/");
        //判断应用实例的baseurl
        if(that.currentInstance.baseUrl!=""&&that.currentInstance.baseUrl!="/"){
            relativePath=that.currentInstance.baseUrl+relativePath;
        }
        newurl = host+ relativePath;
